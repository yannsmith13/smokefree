document.addEventListener("DOMContentLoaded", function() {
    ...
    function buildCalendar(viewDate, quit, today, milestones){
      ...
      for (let i=0; i<totalCells; i++){
        ...
        // mark as clean day if between quit (exclusive) and today (inclusive)
        const sd = startOfDay(d);
        const isAfterQuit = sd > quit;
        const isOnOrBeforeToday = sd <= today;
        const isClean = isAfterQuit && isOnOrBeforeToday;
    ...
    function render(){
      ...
      // If quit date is in the future, clamp to 0 days
      const days = Math.max(0, diffDays(quit, today));
      const total = days * Number(state.dailyRate || 0);

      els.streakDays.textContent = days;
      els.totalEuros.textContent = euro(total);

      els.sinceText.textContent = `${days} jour${days>1?'s':''} sans fumer`;
      els.sinceDetail.textContent = `Arrêt le ${fmtDateFR(quit)} • Aujourd’hui le ${fmtDateFR(today)}`;

      els.kpiDays.textContent = days;
      els.kpiDaysSub.textContent = days === 0 ? 'Départ : aujourd’hui' : 'Continue comme ça 👊';
      els.kpiRate.textContent = euro(Number(state.dailyRate || 0)).replace(',00','');
      els.kpiTotal.textContent = euro(total);
      els.kpiTotalSub.textContent = `≈ ${euro(Number(state.dailyRate||0) * 7)} / semaine`;
      ...
      if (nxt){
        els.nextMilestoneName.textContent = nxt.name;
        const remaining = diffDays(today, nxt.date);
        const totalSpan = Math.max(1, diffDays(quit, nxt.date));
        const doneSpan  = Math.max(0, diffDays(quit, today));
        ...
        els.progressText.textContent = `${Math.min(doneSpan,totalSpan)} / ${totalSpan} jours`;

        const gainToNext = remaining * Number(state.dailyRate || 0);
        els.nextMilestoneGain.textContent = euro(gainToNext);
      } else {
        els.nextMilestoneName.textContent = '—';
        els.nextMilestoneWhen.textContent = '—';
        els.progressBar.style.width = '0%';
        els.progressText.textContent = '—';
        els.nextMilestoneGain.textContent = '—';
      }
      ...
      for (const ms of milestones){
        ...
        if (isPast){
          right.textContent = 'Accompli';
        } else if (isToday){
          right.textContent = 'Aujourd’hui !';
        } else {
          const r = diffDays(today, ms.date);
          right.textContent = `Dans ${r} j`;
        }
      ...
      // Calendar view uses viewDate month
      buildCalendar(startOfDay(new Date(viewDate.getFullYear(), viewDate.getMonth(), 1)), quit, today, milestones);

      // Inputs
      els.quitDate.value = state.quitDate;
      els.dailyRate.value = state.dailyRate;

      // Celebrate if milestone reached today (once)
    ...
    document.getElementById('reset').addEventListener('click', ()=>{
      ...
      // clean any celebrated markers too (optional)
